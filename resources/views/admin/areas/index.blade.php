...
<div class="card">
  <h2 class="card-header text-center">Areas</h2>
  <div class="card-body">
    <div class="search mt-2 mb-5">
      <div class="buttons mb-2">
        <a href="/admin/area/create" type="button" class="btn btn-outline-primary btn-sm">+ Add area</a>
      ...
      <table class="table">
        ...
        <tbody class="table-border-bottom-0">
          @foreach ($areas as $area)
            <tr id="deleteRow{{$area->id}}">
              <td>{{ $area->id }}</td>
              <td>{{ $area->name }}</td>
              <td>{{ $area->price }}</td>
              <td class="text-center">
                <div class="btn-group">
                  <form action="/admin/area/{{$area->id}}" method="POST">
                    @method('DELETE')
                    @csrf
                    <a href="/admin/area/{{$area->id}}/edit" type="button" class="btn btn-outline-primary btn-sm">Edit</a> <!-- Added btn-sm class -->
                    <button type="button" onclick="showConfirmationModal({{$area->id}})" class="btn btn-outline-danger btn-sm">Delete</button>
                  </form>                
              </div>
              </td>
            </tr>
